...
void Catalogues::ReadParticlesHDF5(
    ...
    for (uint ifiling = 0; ifiling < filelistprior.size(); ++ifiling) {
        if (parameters.halos) {
            // If sources are HDF5 haloes, we are looking for the file containing the
            // keyword 'hfprop'
            ...
            // If sources are HDF5 DM particles, we are looking for all the files
            // containing the keyword 'shell'
    ...
    if (rank == 0 && !(partlist.size())) {
        std::cout << "# WARNING:  No files detected: pleasure ensure that the halo "
                     "file contains 'hfprop' or that particle files contain '.h5' "
                     "in their name"
                  << std::endl;
    ...
    // Random shuffling of the files (this ensures that different MPI tasks open
    // different files)
    ...
    if (!parameters.halos) {
        ...
        // by the total number of particles within redshifts
    ...
    if (parameters.halos) {
        ...
        if (rank == 0) {
            ...
            std::cout << "# Number of particles wanted : " << parameters.npart << " "
                      << " Total number of particles : " << nparttot << " Fraction "
                      << fraction << " nfiles : " << partlist.size() << std::endl;
        }
#endif
        for (uint ifiling = 0; ifiling < partlist.size(); ++ifiling) {
#ifdef VERBOSE
...
void Catalogues::ReadParticlesASCII(
    ...
    for (uint ifiling = 0; ifiling < filelistprior.size(); ++ifiling) {
        if (parameters.halos) {
            // If sources are ASCII haloes, we are looking for the file containing the
            // keyword 'hfprop'
            found = filelistprior[ifiling].find("hfprop");
            if (found != std::string::npos) {
                partlist.push_back(parameters.sourcedir + filelistprior[ifiling]);
            ...
            // If sources are ASCII DM particles, we are looking for all the .dat
            // files
    ...
    if (rank == 0 && !(partlist.size())) {
        std::cout << "# WARNING:  No files detected: pleasure ensure that the halo "
                     "file contains 'hfprop' or that particle files contain '.dat' "
                     "in their name"
                  << std::endl;
        std::cout << "# Error at file " << __FILE__ << ", line : " << __LINE__
                  << std::endl;
    ...
    if (rank == 0) {
        std::cout << "# Targets : Particles" << std::endl;
        std::cout << "# Number of particles wanted : " << parameters.npart << " "
                  << " Total number of particles : " << caractVect_source.size()
                  << " nfiles : " << partlist.size() << std::endl;
    }
#endif

    // Only take a fraction of the DM particles if we want less that the total
...
/// \param[in,out]  redshifts Vector containing the redshift decomposition for a
/// given source. \param[in,out]  interpRef value for interpolation of the
...
std::array<std::array<double, 2>, 2> Catalogues::newtonMethod2d(
    ...
    if (parameters.beam == "bundle") {
        ...
        if (parameters.plane == "sachs") {
            ...
            std::cout << "# WARNING : with beam = 'bundle', please choose plane = "
                         "'sachs', 'normal' or 'exact'"
                      << std::endl;
            std::cout << "# Error at file " << __FILE__ << ", line : " << __LINE__
                      << std::endl;
            std::terminate();
    ...
    if (result[1][0] > parameters.cat_accuracy) {
        if (parameters.use_previous_catalogues == 0) { // run catalogues
            ...
            std::cout << std::setprecision(17) << "# Direction : " << phi << " "
                      << theta << " result[1][0] : " << result[1][0]
                      << " err : " << err << std::endl;
            ...
            for (double i = -err; i <= err; i += 0.2 * err) {
                for (double j = -err; j <= err; j += 0.2 * err) {
                    ...
                    if (sep_tmp < parameters.cat_accuracy) {
                        std::cout << "Good, stop now " << i << " " << j
                                  << " delta : " << err << " separation : " << sep_tmp
                                  << " best for the moment : " << result[1][0]
                                  << " must be below : " << parameters.cat_accuracy
                                  << std::endl;
                        result[1][0] = sep_tmp;
                        phi_tmp = phi + i;
                        theta_tmp = theta + j;
                        ...
                        // Break second loop
                        break;
                        // If trajectory if better than previously but still not the desired
                        // accuracy
                    } else if (sep_tmp < result[1][0]) {
                        std::cout << "# Better, continue " << i << " " << j
                                  << " delta : " << err << " separation : " << sep_tmp
                                  << " best for the moment : " << result[1][0]
                                  << " must be below : " << parameters.cat_accuracy
                                  << std::endl;
        ...
        // Newton's method for root-finder (since it did not yet converge), using a
    ...
    const double unit_t = scale_factor * scale_factor * parameters.mpc /
...
/// \param[in]      h Dimensionless Hubble parameter
/// \param[in,out]  redshifts Vector containing the redshift decomposition for a
/// given source. \param[in]      interpRef value for interpolation of the
/// bundle. \return         2x2 array with NEW initial angles given after
...
/// \param[in]      observer Observer position.
/// \param[in]      targets_position vector of target positions, velocities and
/// radius. \param[in]      previous_catalogue Previously computed catalogue,
/// used to re-run rejected sources. \param[in]      parameters Parameter
