...
/// \return         2x2 array Aij giving the jacobian from seen angles to true angles
template < int Order, bool RK4, bool Verbose, class Parameter, class Point, class Cosmology, class Octree, class Type > 
std::array< std::array<double, 2>, 2> Lensing::dbetadtheta(const Parameter& parameters, const Point& kiTarget, const Type interpRef, const Point& observer, const Type phiInit, const Type thetaInit, const Type dist, const Cosmology& cosmology, const Octree& octree, const Point& vobs, const Type length){

    std::vector<Point> kiTargets{kiTarget};
    std::vector<Type> interpRefvec{interpRef};
    std::vector<Type> distance{dist};
    return Lensing::dbetadtheta<Order, RK4, Verbose>(parameters, kiTargets, interpRefvec, observer, phiInit, thetaInit, distance, cosmology, octree, vobs, length)[0];
}
...
/// \return         2x2 array Aij giving the jacobian from seen angles to true angles
template < int Order, bool RK4, bool Verbose, class Parameter, class Point, class Cosmology, class Octree, class Type > 
std::vector < std::array< std::array<double, 2>, 2> > Lensing::dbetadtheta(const Parameter& parameters, const std::vector<Point>& kiTargets, const std::vector<Type>& interpRefvec, const Point& observer, const Type phiInit, const Type thetaInit, const std::vector<Type>& dist, const Cosmology& cosmology, const Octree& octree, const Point& vobs, const Type length){
    ...
    rini[1][0] = target[0] - to*e1[0];
    rini[1][1] = target[1] - to*e1[1];
    rini[1][2] = target[2];

    rini[2][0] = target[0] - to*e2[0];
    rini[2][1] = target[1] - to*e2[1];
    rini[2][2] = target[2] - to*e2[2];
...
/// \param[in]      distance scalar comoving distance to stop lensing integration
/// \param[in]      phi initial trajectory angle
/// \param[in]      theta initial trajectory angle
/// \param[in]      trajectory Photon trajectory used to compute the jacobian matrix.
/// \param[in]      octree Octree.
/// \param[in]      length Spatial length in SI units.
/// \return         2x2 array Aij giving the jacobian from seen angles to true angles
template < int Order, bool RK4, bool Verbose, class Type, class Trajectory, template <typename Kind, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container> class Octree, typename Kind, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container > 
std::array< std::array<double, 2>, 2>  Lensing::dbetadtheta_infinitesimal(const Type distance, const Trajectory& trajectory, const Octree< Kind, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const Type length){

    std::vector<Type> dist{distance};
    return Lensing::dbetadtheta_infinitesimal<Order, RK4, Verbose>(dist, trajectory, octree, length)[0];
...
/// \tparam         Element element type
/// \tparam         Container container type
/// \param[in]      dist vector of comoving distances to stop lensing integrations
/// \param[in]      phi initial trajectory angle
/// \param[in]      theta initial trajectory angle
/// \param[in]      trajectory Photon trajectory used to compute the jacobian matrix.
/// \param[in]      octree Octree.
...
/// \return         2x2 array Aij giving the jacobian from seen angles to true angles
template < int Order, bool RK4, bool Verbose, class Type, class Trajectory, template <typename Kind, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container> class Octree, typename Kind, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container > 
std::vector< std::array< std::array<double, 2>, 2> > Lensing::dbetadtheta_infinitesimal(const std::vector<Type>& dist, const Trajectory& trajectory, const Octree< Kind, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const Type length){
	...
	// Convert to angular derivative (see Barreira et al. 2016, eq 39-41). d1d2p corrected : need to go from orthonormal vector frame to coordinate frame.
	d1d1p = sp*sp*dxxp+cp*cp*dyyp-s2p*dxyp;
	d2d2p = cp*cp*ct*ct*dxxp+sp*sp*ct*ct*dyyp+st*st*dzzp+s2p*ct*ct*dxyp-sp*s2t*dyzp-cp*s2t*dxzp;
	d1d2p = ct*cp*sp*(dyyp-dxxp)+(cp*cp-sp*sp)*ct*dxyp+sp*st*dxzp-cp*st*dyzp;
    ...
    for(unsigned int j = 0; j < size; j++){
	photon[0].chi() = dist[j];
	marked = std::distance(std::begin(trajectory), std::upper_bound(std::begin(trajectory), std::end(trajectory), photon[0], [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.chi() < second.chi();}));
	firstid = marked - (marked > 0);
