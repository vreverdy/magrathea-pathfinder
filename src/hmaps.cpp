...
int main(int argc, char* argv[])
{
    ...
    // Execution
    if(rank == 0)
        std::cout<<"# Compute Healpix maps, nside = "<<parameters.nside<<std::endl;
	...
	z_stop_vec[i] = parameters.z_stop_min + i*(parameters.z_stop_max - parameters.z_stop_min)/(parameters.nb_z_maps - (parameters.nb_z_maps > 1));
    }
    ...
    // For each map type, assign a unique index. 'Lensing' types produce multiple maps
	...
	if(map_components[i] == "lensing"){
	    ...
	    if(parameters.jacobiantype=="bundle"){
	        if(rank == 0){
	            ...
	            std::cout<<"Index: "<<nmaps+2<<", component: gamma2 ('lensing')"<<std::endl;
	            std::cout<<"Index: "<<nmaps+3<<", component: w ('lensing')"<<std::endl;
	        ...
	        if(rank == 0){
	            ...
	            std::cout<<"Index: "<<nmaps+1<<", component: gamma1 ('lensing')"<<std::endl;
	            std::cout<<"Index: "<<nmaps+2<<", component: gamma2 ('lensing')"<<std::endl;
	            std::cout<<"Index: "<<nmaps+3<<", component: inverse magnification ('lensing')"<<std::endl;	
	        }
	        nmaps += 3;
	    } else{
	    ...
	    if(rank == 0){
	        std::cout<<"Index: "<<nmaps<<", component: kappa ('lensing_born') WARNING: with lensing_born we compute the jacobian matrix with an infinitesimal beam (no bundle method available)"<<std::endl;
	        std::cout<<"Index: "<<nmaps+1<<", component: gamma1 ('lensing_born')"<<std::endl;
	        std::cout<<"Index: "<<nmaps+2<<", component: gamma2 ('lensing_born')"<<std::endl;
	        std::cout<<"Index: "<<nmaps+3<<", component: inverse magnification ('lensing_born')"<<std::endl;	
	    }
	    nmaps += 3;    
	} else if(map_components[i] == "deflection"){
	    index_components[i] = 13;
	    if(rank == 0){
	        std::cout<<"Index: "<<nmaps<<", component: alpha1 ('deflection') [dphi in spherical coordinates]"<<std::endl;
	        std::cout<<"Index: "<<nmaps+1<<", component: alpha2 ('deflection') [dtheta in spherical coordinates]"<<std::endl;
	    }
	    nmaps++;
	} else{
	    if(rank == 0)
	        std::cout<<"Index: "<<nmaps<<", component: '"<<map_components[i]<<"'"<<std::endl;
		...
		std::cout<<"# WARNING: Need good name for map_components"<<std::endl;
	        std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
	        std::terminate();
	    }
	} 
    ...
    pixel.shrink_to_fit();
#ifdef VERBOSE
    std::cout<<"# Pixels distributed : rank "<<rank<<" takes care of "<<pixel.size()<<" pixels"<<std::endl;
#endif
    point observer; 
    observer[0] = 0;
    observer[1] = 0;
    ...
    std::vector<double> thomo(parameters.nb_z_maps), rhomo(parameters.nb_z_maps), lambdahomo(parameters.nb_z_maps), redshifthomo(parameters.nb_z_maps), ahomo(parameters.nb_z_maps);

    // For each redshift, estimate the homogeneous quantities [scale factor, redshift, comoving distance, conformal time, affine parameter]
    for(unsigned int i = 0; i < parameters.nb_z_maps; i++){
	photon.redshift() = z_stop_vec[i];
	const unsigned long int marked = std::distance(std::begin(reference), std::upper_bound(std::begin(reference), std::end(reference), photon, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.redshift() < second.redshift();}));
	const unsigned long int firstid = marked - (marked > 0);
	const double f = (reference[firstid+1].redshift() - photon.redshift())/(reference[firstid+1].redshift() - reference[firstid].redshift());
	thomo[i] = reference[firstid].t()*f + reference[firstid+1].t()*(1-f);
	rhomo[i] = reference[firstid].chi()*f + reference[firstid+1].chi()*(1-f);
	lambdahomo[i] = reference[firstid].lambda()*f + reference[firstid+1].lambda()*(1-f);
	...
	if(parameters.stop_ray == "t"){
	    ...
	    std::cout<<"Need good interpolation 't, r, lambda, a, redshift or ell'"<<std::endl;
	    std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
	    std::terminate();
	}

	if(rank == 0){
	    std::cout<<std::setprecision(17)<<"# Homogeneous quantities : z = "<<redshifthomo[i]<<" a = "<<ahomo[i]<<" t = "<<thomo[i]<<" chi = "<<rhomo[i]<<" lambda = "<<lambdahomo[i]<<std::endl;
	}
    }
...
#ifdef VERBOSE
   	    std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", now computing Healpix maps"<<std::endl;
#endif                  
...
#ifdef VERBOSE
    if(rank == 0)
	std::cout<<"# Maps at inhomogeneous redshift(s). Velocity field computed with "<<parameters.velocity_field<<std::endl;
#endif
	...
	if (icone%static_cast<uint>(ntasks) == static_cast<uint>(rank)) {
#ifdef VERBOSE
	    std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", getting particles..."<<std::endl;
#endif
	    // Load octree from binary file
	    Miscellaneous::loadOctree(icone, octree, conefile);
	    // WARNING ! Set rho (density) to zero in each cell of the octree; 
	    Utility::parallelize(octree.size(), [=, &octree](const uint i){std::get<1>(octree[i]).rho() = 0;});
		...
		//Miscellaneous::TicketizeFunction(rank, ntasks, [=, &shellList, &pos_part, &vel_part]{ // Uncomment in case of IO problems
		...
		if(pos_part.size() > 0){
#ifdef VERBOSE
       		    std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", file : "<<ifile<<"/"<<shellList.size()<<" npart "<<pos_part.size()/3<<" : "<<shellList[ifile]<<std::endl;
#endif
		    // From particles, compute the velocity field with CIC or TSC scheme
		    if(parameters.velocity_field == "cic"){
	    	        Hmaps::CreateOctreeVelocityWithCIC(octree, pos_part, vel_part); // octree with velocity field using CIC interpolation 
#ifdef VERBOSE
		        std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", Octree created with CIC"<<std::endl;
#endif
		    } else if(parameters.velocity_field == "tsc"){
	    	        Hmaps::CreateOctreeVelocityWithTSC(octree, pos_part, vel_part); // octree with velocity field using TSC interpolation 
#ifdef VERBOSE
		        std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", Octree created with TSC"<<std::endl;
#endif
		    } else{
    		        std::cout<<"No velocity field computed, please type cic or tsc"<<std::endl;
			std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
			std::terminate();
		    }
		}
	    ...
	    // Fill map
#ifdef VERBOSE
	    std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", now computing Healpix maps, ntrajectory = "<<ntrajectories[itraj]<<std::endl;
#endif
	    // Fill Healpix maps with ray-tracing routines    
...
#ifdef VERBOSE
	    std::cout<<"# Rank "<<rank<<" using cone "<<icone<<", Healpix maps computed"<<std::endl;
#endif
	    firsttrajectory += ntrajectories[itraj];
	    itraj++;
	}
...
#ifdef VERBOSE
    std::cout<<"# Rank "<<rank<<" : Healpix maps filled, now waiting for other procs to finalize "<<std::endl;
#endif 
    ...
    for(uint iz = 0; iz < parameters.nb_z_maps; iz++){
	    ...
	    // Write the outputs if all the tasks contain a full map, or if the total number of maps are contained
	    //std::cout<<"rank = "<<rank<<" map "<<nmaps*iz + i<<" ntasks "<<ntasks<<" ratio "<<(nmaps*iz + i) % static_cast<uint>(ntasks)<<" max "<<static_cast<uint>(nmaps*parameters.nb_z_maps - 1)<<" my "<<my_iz<<" "<<my_i<<std::endl;
	    if (((nmaps*iz + i) % static_cast<uint>(ntasks) == static_cast<uint>(ntasks - 1)) | ((nmaps*iz + i) == static_cast<uint>(nmaps*parameters.nb_z_maps - 1)) ) {
		// Select tasks
		if(my_iz != -1){
	            // Clean the map
 	            for (long pix = 0; pix < npix; pix++){
		        // If pixel is not normal (i.e. zero, infinity or NaN), then put undefined value 
	                if (!std::isnormal(mapdata2[pix])){
		            mapdata2[pix] = Healpix_undef; 
		        }
	            }	
#ifdef VERBOSE
	            std::cout << " #-- Writing output with rank = "<<rank<<" redshift = "<<z_stop_vec[my_iz]<<" id = "<<my_i<< std::endl;
#endif
	            std::string outputfilename; 
		    outputfilename = parameters.outputdir + parameters.outputprefix + "_nside" + std::to_string(parameters.nside) + "_" + std::to_string(my_i) +"_z_"+std::to_string(z_stop_vec[my_iz])+"_"+parameters.stop_ray+".fits";
		    // Write Healpix map
