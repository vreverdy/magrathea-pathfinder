...
class Hmaps {
	...
	// Target
        template < class Parameter, typename Integer, class Cones>  static void getPixels_per_cone(const Parameter& parameters, const Integer npix, std::vector<Integer>& pixel, std::vector<unsigned int>& ntrajectories, const unsigned int iconerank, const Cones& cones);
        template < class Parameter, typename Integer, class Cones>  static void getPixels_per_cone2(const Parameter& parameters, const Integer npix, std::vector<Integer>& pixel, std::vector<unsigned int>& ntrajectories, const unsigned int iconerank, const Cones& cones);

	// Fill particles
#ifdef VELOCITYFIELD
	template < class Parameter, class Cone, typename Type1, typename Type2 > static void fill_particles_vectors(const Parameter& parameters, const std::array< std::array< double, 3 >, 3 >& rotm1, const Cone& cone, const std::vector< std::string >& shellList, std::vector<Type1>& pos_part, std::vector<Type1>& vel_part, const std::vector<Type2>& z_stop_vec, const double thetay, const double thetaz);
	template < class Parameter, class Cone, typename Type1, typename Type2 > static void fill_particles_vectors(const Parameter& parameters, const std::array< std::array< double, 3 >, 3 >& rotm1, const Cone& cone, const std::string shellList, std::vector<Type1>& pos_part, std::vector<Type1>& vel_part, const std::vector<Type2>& z_stop_vec, const double thetay, const double thetaz);
	// Velocity field octree
	template < typename Type1, template <typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container> class Octree, typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container > static void CreateOctreeVelocityWithCIC(Octree< Type, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const std::vector<Type1>& pos_part, const std::vector<Type1>& vel_part);
	template < typename Type1, template <typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container> class Octree, typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container > static void CreateOctreeVelocityWithTSC(Octree< Type, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const std::vector<Type1>& pos_part, const std::vector<Type1>& vel_part);
#endif
	// Fill maps
	template < class Parameter, class Octree, class Map, class Pixel, class Cosmology, class Point, typename Integer, typename Real > static void FillMapPropagate(const Parameter& parameters, const Integer ntrajectory, const Integer firsttrajectory, const Octree& octree, const Point& vobs, Map& map, const Integer nmaps, const Pixel& pixel, const Cosmology& cosmology, const point& observer, const Real length, const std::vector<Real>& z_stop_vec);

        template < class Parameter, class Octree, class Map, class Pixel, class Cosmology, class Point, typename Integer, typename Real >  static void FillMap(const Parameter& parameters, const std::vector<std::string>& map_components, const std::vector<unsigned int>& index_components, const Integer ntrajectory, const Integer firsttrajectory, const Octree& octree, const Point& vobs, Map& map, const unsigned int nmaps, const Pixel& pixel, const Cosmology& cosmology, const point& observer, const Real length, const std::vector<Real>& interpRefvec, const std::vector<Real>& rhomo, const std::vector<Real>& thomo, const std::vector<Real>& lambdahomo, const std::vector<Real>& redshifthomo, const std::vector<Real>& ahomo);
...
/// \param[in,out]  pixel Vector of pixels inside cones that are treated by the same MPI task.
/// \param[in]	    ntrajectories Number of pixels per cone.
...
void Hmaps::getPixels_per_cone(const Parameter& parameters, const Integer npix, std::vector<Integer>& pixel, std::vector<unsigned int>& ntrajectories, const unsigned int iconerank, const Cones& cones)
{   
	...
	if (ang1 <= cones[iconerank].angle()){
	    closest = true;
	    // Loop over all the other cones to find if there is another one that is closer
    ...
    pixeltmp.erase(std::remove(std::begin(pixeltmp), std::end(pixeltmp), -1), std::end(pixeltmp));
    // Count number of pixels inside the cone
    ntrajectories.back() = pixeltmp.size();
    // Insert pixels in the vectors for a single MPI task which will take care of several cones
...
/// return  	    Filled vector of targets inside cones for a given MPI task
template < class Parameter, typename Integer, class Cones>  
void Hmaps::getPixels_per_cone2(const Parameter& parameters, const Integer npix, std::vector<Integer>& pixel, std::vector<unsigned int>& ntrajectories, const unsigned int iconerank, const Cones& cones)
{   
    // Fill temporary pixel vector with -1
    std::vector<long> pixeltmp(npix, -1);
    ntrajectories.push_back(0);
	...
	// Check if pixel is inside the cone
	    ...
	    // Compute scalar product of cone base and pixel direction
            for (unsigned int idim = 0; idim < 3; ++idim) {
                length += (cones[iconerank].base(idim)-cones[iconerank].vertex(idim))*(vec[idim]-cones[iconerank].vertex(idim));
            }
            length /= cones[iconerank].template pow<2>(cones[iconerank].length());
	    // Compute the ditance between cone base and pixel
            for (unsigned int idim = 0; idim < 3; ++idim) {
                reference += cones[iconerank].template pow<2>(vec[idim]-(cones[iconerank].vertex(idim)+(cones[iconerank].base(idim)-cones[iconerank].vertex(idim))*length));
            }
		...
		distance = 0;
		    ...
		    // Compute scalar product of other cone base and pixel direction
                    for (unsigned int idim = 0; idim < 3; ++idim) {
                        length += (cones[icone].base(idim)-cones[iconerank].vertex(idim))*(vec[idim]-cones[icone].vertex(idim));
                    }
		    if(!(length < 0)){
                        length /= cones[icone].template pow<2>(cones[icone].length());
	    		// Compute the ditance between other cone base and pixel
                        for (unsigned int idim = 0; idim < 3; ++idim) {
                            distance += cones[icone].template pow<2>(vec[idim]-(cones[icone].vertex(idim)+(cones[icone].base(idim)-cones[icone].vertex(idim))*length));
                        }
...
void Hmaps::fill_particles_vectors(const Parameter& parameters, const std::array< std::array< double, 3 >, 3 >& rotm1, const Cone& cone, const std::vector< std::string >& shellList, std::vector<Type1>& pos_part, std::vector<Type1>& vel_part, const std::vector<Type2>& z_stop_vec, const double thetay, const double thetaz){

    unsigned long long int marker(0);
    const double c = magrathea::Constants<double>::c();

#ifdef VERBOSE
    std::cout<<"# Now loading the position and velocity of particles from the following files : "<<std::endl;
#endif
	...
	// Must be chosen so the maximum observed redshift is far enough (take dz = 5e-3. Compare to redshift step between shells and put int(dz/redshiftstep) + 1)
        amaxing += 4*da;
        amining -= 4*da;
#ifdef VERBOSE
	...
	for(uint iz = 0; iz < z_stop_vec.size(); iz++){
	    ...
	    // If file is inside the buffer zone of any reference redshift, then get the particles
            if(scale_factor <= amining  &&  scale_factor >= amaxing){
                ...
                std::transform(std::begin(vel_part)+marker, std::end(vel_part), std::begin(vel_part)+marker, std::bind1st(std::multiplies<double>(), factor)); // SI units
	...
	std::vector<Type1> vel_part_tmp = vel_part;
        Utility::parallelize(pos_part.size()/3, [=, &pos_part, &vel_part, &pos_part_tmp, &vel_part_tmp, &rotm1](const uint i){
	    pos_part[3*i] = pos_part_tmp[3*i]*rotm1[0][0] + pos_part_tmp[3*i+1]*rotm1[0][1] + pos_part_tmp[3*i+2]*rotm1[0][2];
	    pos_part[3*i+1] = pos_part_tmp[3*i]*rotm1[1][0] + pos_part_tmp[3*i+1]*rotm1[1][1] + pos_part_tmp[3*i+2]*rotm1[1][2];
	    pos_part[3*i+2] = pos_part_tmp[3*i]*rotm1[2][0] + pos_part_tmp[3*i+1]*rotm1[2][1] + pos_part_tmp[3*i+2]*rotm1[2][2];
	    vel_part[3*i] = vel_part_tmp[3*i]*rotm1[0][0] + vel_part_tmp[3*i+1]*rotm1[0][1] + vel_part_tmp[3*i+2]*rotm1[0][2];
	    vel_part[3*i+1] = vel_part_tmp[3*i]*rotm1[1][0] + vel_part_tmp[3*i+1]*rotm1[1][1] + vel_part_tmp[3*i+2]*rotm1[1][2];
	    vel_part[3*i+2] = vel_part_tmp[3*i]*rotm1[2][0] + vel_part_tmp[3*i+1]*rotm1[2][1] + vel_part_tmp[3*i+2]*rotm1[2][2];
	});
    }
}
// Fill vectors of particles
/// \brief          Rewrite particle in vector.
/// \details        Rewrite particle position/velocity from HDF5 in a vector. Single file version
/// \tparam         Parametr Parameter type
/// \tparam         Cone cone type
/// \tparam         Type1 type1 type
/// \tparam         Type2 type2 type
/// \param[in]      parameters Parameter structure.
...
/// \param[in]      thetaz Semi-angle for solid angle in direction z
template < class Parameter, class Cone, typename Type1, typename Type2 > 
void Hmaps::fill_particles_vectors(const Parameter& parameters, const std::array< std::array< double, 3 >, 3 >& rotm1, const Cone& cone, const std::string shellList, std::vector<Type1>& pos_part, std::vector<Type1>& vel_part, const std::vector<Type2>& z_stop_vec, const double thetay, const double thetaz){
...
void Hmaps::CreateOctreeVelocityWithCIC(Octree< Type, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const std::vector<Type1>& pos_part, const std::vector<Type1>& vel_part){
	    ...
	    // Loop over the 8 neighboring cells of a given particle
	        ...
	        for(int iy = -1; iy <= 1; iy += 2){
		        ...
		        if(std::get<0>(*(std::begin(octree)+marker-(marker > 0))) == idxvertex){
		            ...
		            std::get<1>(octree[marker - (marker > 0)]).vx() += vel_part[3*i]*vratio;
		            std::get<1>(octree[marker - (marker > 0)]).vy() += vel_part[3*i+1]*vratio;
		            std::get<1>(octree[marker - (marker > 0)]).vz() += vel_part[3*i+2]*vratio;
		        }		
	            }
	        }  
            }
...
/// \param[in]      vel_part Velocity of particles
template < typename Type1, template <typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container> class Octree, typename Type, class Index, class Data, unsigned int Dimension, class Position, class Extent, class Element, class Container > 
void Hmaps::CreateOctreeVelocityWithTSC(Octree< Type, Index, Data, Dimension, Position, Extent, Element, Container >& octree, const std::vector<Type1>& pos_part, const std::vector<Type1>& vel_part){
...
/// \param[in]      index_components Vector containing the index relative to the map types.
...
/// \param[in]      interpRefvec vector of interpolations at a given quantity for a given surface
/// \param[in]      rhomo vector of FLRW distance at the redshifts of interest
/// \param[in]      thomo vector of FLRW time at the redshifts of interest
/// \param[in]      lambdahomo vector of FLRW affine parameter at the redshifts of interest
/// \param[in]      redshifthomo vector of FLRW redshift at the redshifts of interest
/// \param[in]      ahomo vector of FLRW scale factor at the redshifts of interest
template < class Parameter, class Octree, class Map, class Pixel, class Cosmology, class Point, typename Integer, typename Real > 
void Hmaps::FillMap(const Parameter& parameters, const std::vector<std::string>& map_components, const std::vector<unsigned int>& index_components, const Integer ntrajectory, const Integer firsttrajectory, const Octree& octree, const Point& vobs, Map& map, const unsigned int nmaps, const Pixel& pixel, const Cosmology& cosmology, const point& observer, const Real length, const std::vector<Real>& interpRefvec, const std::vector<Real>& rhomo, const std::vector<Real>& thomo, const std::vector<Real>& lambdahomo, const std::vector<Real>& redshifthomo, const std::vector<Real>& ahomo){

    // Loop over all the pixels in the cone
    Utility::parallelize(ntrajectory, [=, &parameters, &map_components, &index_components, &ntrajectory, &firsttrajectory, &octree, &vobs, &map, &nmaps, &pixel, &cosmology, &observer, &length, &interpRefvec, &rhomo, &thomo, &lambdahomo, &redshifthomo, &ahomo](const unsigned int itrajectory){

	const uint itrajectorys = itrajectory + firsttrajectory;
        magrathea::Evolution<Photon<double, 3> > trajectorycenter, trajectorycenter_born;
	Photon<double, 3> photoncenter;
	std::vector< std::array< std::array<double, 2>, 2 > > jacobian(parameters.nb_z_maps), jacobian_born(parameters.nb_z_maps);
	std::vector< uint > firstid(parameters.nb_z_maps);
	std::vector<Real> interpRefvecBundle(parameters.nb_z_maps), f(parameters.nb_z_maps), distance(parameters.nb_z_maps);
	double phi(0), theta(0);
	unsigned int marked(0);
	double vec1[3];
	const bool only_born = (index_components.size() == 1 && index_components[0] == 1);
	const bool compute_born = std::find(map_components.begin(), map_components.end(), "lensing_born") != map_components.end();
	const bool compute_lensing = std::find(map_components.begin(), map_components.end(), "lensing") != map_components.end();
	std::vector<point> kiTargets(parameters.nb_z_maps), posTargets(parameters.nb_z_maps);
        // Convert pixel to 3D vector
	...
	if(!only_born){
	    ...
	    // Integrate photon trajectory
            ...
            //Integrator::integrate(trajectorycenter, cosmology, octree, vobs, length, parameters.nsteps); // Uncomment to compute upper surface (with stop_rays == redshift)
	    ...
	    for(uint i = 0; i < parameters.nb_z_maps; i++){
		    ...
		    marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.chi() < second.chi();}));
	            firstid[i] = marked - (marked > 0);
	            f[i] = (trajectorycenter[firstid[i]+1].chi() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].chi() - trajectorycenter[firstid[i]].chi());
                } else if(parameters.stop_ray == "lambda"){
		    photoncenter.lambda() = interpRefvec[i];
		    marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.lambda() < second.lambda();}));
	            firstid[i] = marked - (marked > 0);
	            f[i] = (trajectorycenter[firstid[i]+1].lambda() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].lambda() - trajectorycenter[firstid[i]].lambda());
                } else if(parameters.stop_ray == "t"){
		    photoncenter.t() = interpRefvec[i];
		    marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.t() < second.t();}));
	            firstid[i] = marked - (marked > 0);
	            f[i] = (trajectorycenter[firstid[i]+1].t() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].t() - trajectorycenter[firstid[i]].t());
                } else if(parameters.stop_ray == "redshift"){
		    photoncenter.redshift() = interpRefvec[i];
		    marked = std::distance(std::begin(trajectorycenter), std::find_if(std::begin(trajectorycenter), std::end(trajectorycenter), [=, &photoncenter](const Photon<double, 3>& first){return first.redshift() > photoncenter.redshift();})); // lower surface
		    //marked = std::distance(std::begin(trajectorycenter), std::find_if(std::rbegin(trajectorycenter), std::rend(trajectorycenter), [=, &photoncenter](const Photon<double, 3>& first){return first.redshift() < photoncenter.redshift();}).base()); // Uncomment to compute upper surface (with stop_rays == redshift)
	            firstid[i] = marked - (marked > 0); 
	            f[i] = (trajectorycenter[firstid[i]+1].redshift() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].redshift() - trajectorycenter[firstid[i]].redshift());
                } else if(parameters.stop_ray == "a"){
		    photoncenter.a() = interpRefvec[i];
		    marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.a() > second.a();}));
	            firstid[i] = marked - (marked > 0); 
	            f[i] = (trajectorycenter[firstid[i]+1].a() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].a() - trajectorycenter[firstid[i]].a());
                } else if(parameters.stop_ray == "s"){
		    photoncenter.s() = interpRefvec[i];
		    marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.s() < second.s();}));
	            firstid[i] = marked - (marked > 0);
	            f[i] = (trajectorycenter[firstid[i]+1].s() - interpRefvec[i])/(trajectorycenter[firstid[i]+1].s() - trajectorycenter[firstid[i]].s());
                }
	    }
	    ...
	    if(compute_lensing){
	        for(uint i = 0; i < parameters.nb_z_maps; i++){ // different maps
	            // Get position of photon at the surface and the plane (screen for bundle method)
	            posTargets[i][0] = trajectorycenter[firstid[i]].x()*f[i] + trajectorycenter[firstid[i]+1].x()*(1-f[i]);
		    ...
		    if (parameters.jacobiantype == "bundle"){
	                if (parameters.plane == "sachs"){
	                    ...
	                    kiTargets[i][2] = trajectorycenter[firstid[i]].dzdl()*f[i] + trajectorycenter[firstid[i]+1].dzdl()*(1-f[i]);
		    ...
		    if (parameters.jacobiantype == "bundle"){
	                if(parameters.stop_bundle == "lambda"){
	                    ...
	                    interpRefvecBundle[i] = trajectorycenter[firstid[i]].chi()*f[i] + trajectorycenter[firstid[i]+1].chi()*(1-f[i]); ; 
	                } else if(parameters.stop_bundle == "redshift"){
	                    interpRefvecBundle[i] = trajectorycenter[firstid[i]].redshift()*f[i] + trajectorycenter[firstid[i]+1].redshift()*(1-f[i]); ; 		
	                } else if(parameters.stop_bundle == "a"){
	                    interpRefvecBundle[i] = trajectorycenter[firstid[i]].a()*f[i] + trajectorycenter[firstid[i]+1].a()*(1-f[i]); 
	                } else if(parameters.stop_bundle == "t"){
	                    interpRefvecBundle[i] = trajectorycenter[firstid[i]].t()*f[i] + trajectorycenter[firstid[i]+1].t()*(1-f[i]); 	
	                } else if(parameters.stop_bundle == "plane"){
		            interpRefvecBundle[i] = kiTargets[i][0]*posTargets[i][0] + kiTargets[i][1]*posTargets[i][1] + kiTargets[i][2]*posTargets[i][2]; 
	                } else{
	...
	// Check if we need to compute the lensing jacobian matrix with Born approximation
	if(compute_born){
	    photoncenter = Integrator::launch(observer[0], observer[1], observer[2], vec1[0], vec1[1], vec1[2]);
	    ...
	    jacobian_born = Lensing::dbetadtheta_infinitesimal(rhomo, trajectorycenter_born, octree, length);
	}
	...
	for(uint iz = 0; iz < parameters.nb_z_maps; iz++){ // different maps
	    ...
	    if( (compute_born && trajectorycenter_born.size() == 1) || (!only_born && (firstid[iz] == trajectorycenter.size() - 1 || firstid[iz] == 0))){
		...
		// Loops over map types
			...
			if(parameters.jacobiantype == "bundle"){
			    ...
			    if(a11 == 42){
			        ...
			        break;		    
	                        ...
	                        const double gamma1 = -0.5*(std::cos(w)*(a11-a22) + std::sin(w)*(a12+a21));
	                        const double gamma2 = -0.5*(std::cos(w)*(a12+a21) + std::sin(w)*(a22-a11));
			    ...
			    if(a11 == 42){
			        ...
			        map[nmaps*iz + icomp+ 1][pixel[itrajectorys]] = gamma1; 
			        map[nmaps*iz + icomp+ 2][pixel[itrajectorys]] = gamma2; 
			        map[nmaps*iz + icomp + 3][pixel[itrajectorys]] = invmagnification;
			        icomp += 3;
			    }
			}
		    } else if(index_components[j] == 1){
	                const double a11(jacobian_born[iz][0][0]), a12(jacobian_born[iz][0][1]), a22(jacobian_born[iz][1][1]);
			// Check if jacobian is good
			if(a11 == 42){
		            for(uint i = 0; i < nmaps; i++){
		                map[nmaps*iz + i][pixel[itrajectorys]] = 0;
			...
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].chi()*f[iz] + trajectorycenter[firstid[iz]+1].chi()*(1-f[iz]))/rhomo[iz] - 1;
		    } else if (index_components[j] == 3){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].lambda()*f[iz] + trajectorycenter[firstid[iz]+1].lambda()*(1-f[iz]))/lambdahomo[iz] - 1;
		    } else if (index_components[j] == 4){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].t()*f[iz] + trajectorycenter[firstid[iz]+1].t()*(1-f[iz]))/thomo[iz] - 1;
		    } else if (index_components[j] == 5){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].a()*f[iz] + trajectorycenter[firstid[iz]+1].a()*(1-f[iz]))/ahomo[iz] - 1;
		    } else if (index_components[j] == 6){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].redshift()*f[iz] + trajectorycenter[firstid[iz]+1].redshift()*(1-f[iz]))/redshifthomo[iz] - 1;
		    } else if (index_components[j] == 7){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].s()*f[iz] + trajectorycenter[firstid[iz]+1].s()*(1-f[iz]))/rhomo[iz] - 1;
		    } else if (index_components[j] == 8){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].isw()*f[iz] + trajectorycenter[firstid[iz]+1].isw()*(1-f[iz]));
		    } else if (index_components[j] == 9){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].rho()*f[iz] + trajectorycenter[firstid[iz]+1].rho()*(1-f[iz]));
		    } else if (index_components[j] == 10){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (*std::max_element(std::begin(trajectorycenter), std::begin(trajectorycenter)+firstid[iz], [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.rho() < second.rho();})).rho();
		    } else if (index_components[j] == 11){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = firstid[iz];
		    } else if (index_components[j] == 12){
			map[nmaps*iz + icomp][pixel[itrajectorys]] = (trajectorycenter[firstid[iz]].phi()*f[iz] + trajectorycenter[firstid[iz]+1].phi()*(1-f[iz]));
			...
			if(compute_lensing){
			    ...
			    double x = trajectorycenter[firstid[iz]].x()*f[iz] + trajectorycenter[firstid[iz]+1].x()*(1-f[iz]);
			    double y = trajectorycenter[firstid[iz]].y()*f[iz] + trajectorycenter[firstid[iz]+1].y()*(1-f[iz]);
			    double z = trajectorycenter[firstid[iz]].z()*f[iz] + trajectorycenter[firstid[iz]+1].z()*(1-f[iz]);
...
/// \param[in]      observer Observer position 
/// \param[in]      length R.U to S.I units for length
/// \param[in]      z_stop_vec Vector containing redshifts at which we compute scalar quantities
template < class Parameter, class Octree, class Map, class Pixel, class Cosmology, class Point, typename Integer, typename Real > 
void Hmaps::FillMapPropagate(const Parameter& parameters, const Integer ntrajectory, const Integer firsttrajectory, const Octree& octree, const Point& vobs, Map& map, const Integer nmaps, const Pixel& pixel, const Cosmology& cosmology, const point& observer, const Real length, const std::vector<Real>& z_stop_vec){
    ...
    std::vector<double> f_ref(parameters.nb_z_maps), distance_ref(parameters.nb_z_maps);
    Octree homotree;
    // Create homogeneous octree
    homotree.assign(parameters.ncoarse/2., zero);
    // Compute FLRW ray in the homogeneous octree
    reference = Integrator::propagate<-1>(photonref, parameters.nbundlemin, parameters.openingmin, real(), parameters.stop_ray, cosmology, homotree, vobs0, length, parameters.nsteps*(1 << (parameters.ncoarse-parameters.ncoarse/2))*2, double()); 

    // Loop over all the redshifts to get angular distance and interpolation factors
    for(uint iz = 0; iz < parameters.nb_z_maps; iz++){ 
	photonref.a() = one/(one + z_stop_vec[iz]);
	const unsigned long int marked = std::distance(std::begin(reference), std::upper_bound(std::begin(reference), std::end(reference), photonref, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.a() > second.a();}));
	...
	distance_ref[iz] = reference[firstid_ref[iz]].distance()*f_ref[iz] + reference[firstid_ref[iz]+1].distance()*(1 - f_ref[iz]);
    }
    // Loop over the pixels 
    Utility::parallelize(ntrajectory, [=, &map, &octree, &distance_ref, &parameters, &cosmology, &pixel](const uint itrajectory){

	const uint itrajectorys = itrajectory + firsttrajectory;
        magrathea::Evolution<Photon<double, 3> > trajectorycenter;
	Photon<double, 3> photoncenter;
	std::vector< std::array< std::array<double, 2>, 2 > > jacobian(parameters.nb_z_maps);
	std::vector< uint > firstid(parameters.nb_z_maps);
	double vec1[3];
	...
	// Propagate spherical bundle in the direction of pixel
        trajectorycenter = Integrator::propagate(photoncenter, parameters.nbundlemin, parameters.openingmin, double(), parameters.stop_ray, cosmology, octree, vobs0, length, parameters.nsteps, amin);

	// Check if trajectory is good
	if(trajectorycenter.size()){
	    // Loop over all the redshifts of reference to get the angular diameter distance
	    for(uint iz = 0; iz < parameters.nb_z_maps; iz++){
	        photoncenter.a() = one/(one + z_stop_vec[iz]);
	        const unsigned long int marked = std::distance(std::begin(trajectorycenter), std::upper_bound(std::begin(trajectorycenter), std::end(trajectorycenter), photoncenter, [](const Photon<double, 3>& first, const Photon<double, 3>& second){return first.a() > second.a();}));
	        ...
	        distance[iz] = trajectorycenter[firstid[iz]].distance()*f[iz] + trajectorycenter[firstid[iz]+1].distance()*(1-f[iz]);
	    }
		...
		// Check if everything is fine
		    ...
		    const double sf = trajectorycenter[firstid[iz]].a()*f[iz] + trajectorycenter[firstid[iz]+1].a()*(1-f[iz]);
		    const double chi = trajectorycenter[firstid[iz]].chi()*f[iz] + trajectorycenter[firstid[iz]+1].chi()*(1-f[iz]);
		    map[nmaps*iz + 0][pixel[itrajectorys]] = dd0;
	            map[nmaps*iz + 1][pixel[itrajectorys]] = 1./sf - 1;
	            map[nmaps*iz + 2][pixel[itrajectorys]] = chi*sf/(distance_ref[iz]/length);
