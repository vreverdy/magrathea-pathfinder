...
void Create_octree::PreparationHDF5_from_cells(
    ...
    // For narrow cones, also need to read the files from a 'minicone', i.e. a
    // small spherical region around the observer (to allow for the first
    // integrations steps)
    ...
    for (uint icone = zero; icone < parameters.ncones; ++icone) {
        if (icone % static_cast<uint>(ntasks) == static_cast<uint>(rank)) {
            ...
            // For narrow cones, read minicone files and put cell information in
            // octree
            if (parameters.isfullsky == 0) {
...
#ifdef VERBOSE
            std::cout << "# Before corrections, Cone : " << icone
                      << " Octree size : " << octree.size() << std::endl;
#endif
...
#ifdef VERBOSE
            std::cout << "# Before Update , Cone : " << icone
                      << " Octree size : " << octree.size() << std::endl;
#endif
            octree.update();
...
/// \tparam         Element element type
/// \tparam         Container container type
/// \param[in,out]  continue_refining Bolean which controls if we continue
/// refinement \param[in,out]  octree Octree be to filled using particles
/// \param[in]      parameters Parameters structure
...
void Create_octree::CreateOctreeWithCIC(
    ...
    // If it is still possible to refine (density high enough and maximum
    // refinement level not reached yet)
    if (continue_refining) {
        for (unsigned int i = 0; i < size; i++) {
            // Refine cell (add 8 son cells in the octree) if the mass in one cell
