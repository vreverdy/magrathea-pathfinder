...
void Generate_cones::GenerateFullskyCones(const Integer ncones, std::vector< Cone >& cone, std::vector< Cone >& coneIfRot, Sphere<Dimension, Vector, Scalar>& sphere){
...
void Generate_cones::GenerateNarrowCones(const Parameter& parameters, std::vector< Cone >& cone, std::vector< Cone >& coneIfRot, Sphere<Dimension, Vector, Scalar>& sphere, std::array< std::array< double, 3 >, 3 >& rotm1, Scalar& thetay, Scalar& thetaz){
	...
	if(std::abs(phi) < thetay - parameters.buffer && theta >  pi/2 - thetaz + parameters.buffer && theta < pi/2 + thetaz - parameters.buffer){
	    iloop++;
	}
    } 
    float irecuploop = 0;
