...
// In miscellaneous.h we use std::sample. For versions of gcc lower than 7.x, please use std::experimental::sample. This can be done in the Makefile
...
int main(int argc, char* argv[])
{
    ...
    reference.fullclear();
    for (uint icone = parameters.firstcone; icone < parameters.lastcone + one; ++icone) {
	    ...
	    if(parameters.use_previous_catalogues){
                Miscellaneous::ReadFromCat(icone, parameters, previous_catalogue); // For 'rejected', read the id here, and choose only those that are inside the cone
#ifdef VERBOSE
		std::cout<<"# Rank "<<rank<<" cone "<<icone<<" number of sources in the cone : "<<previous_catalogue.size()<<std::endl;
#endif
		if(previous_catalogue.size() == 0){
		    std::cout<<"# Rank "<<rank<<" cone "<<icone<<" no data, skip this cone"<<std::endl;
		    continue;
		}
	    }
	    // Load octree
		...
		// Read source files (usually depends on some convention in the filenames)
		if(parameters.typefile == 1){
		    ...
		    if(rank == 0){
			std::cout<<"Targets are only available with the HDF5 or ASCII format"<<std::endl;
		...
		targets_position = Miscellaneous::getTargets(caractVect_source, cone[icone], cone);
#ifdef VERBOSE
		std::cout<<"# Rank "<<rank<<" cone "<<icone<<" HALOS : number of sources in the cone : "<<targets_position.size()<<" of "<<caractVect_source.size()<<std::endl;
#endif
		...
		if(parameters.use_previous_catalogues == 2){
		    ...
		    // Sort sources from previously computed catalogue with index
                    std::sort(previous_catalogue.begin(), previous_catalogue.end(), [](const std::array< double, 18 >& a, const std::array< double, 18 >& b) {return a[0] < b[0];});
		    // Sort sources from full dataset with index
                    std::sort(targets_position.begin(), targets_position.end(), [](const std::array< double, 8 >& a, const std::array< double, 8 >& b) {return a[6] < b[6];});
		     
		    for(uint i = 0; i < previous_catalogue.size(); i++){
		        std::array< double, 8 > point_tmp;
		        point_tmp[6] = previous_catalogue[i][0];
		        const unsigned long int marked = std::distance(std::begin(targets_position), std::upper_bound(std::begin(targets_position), std::end(targets_position), point_tmp, [](const std::array< double, 8 >& first, const std::array< double, 8 >& second){return first[6] < second[6];}));
			// If we find a source that was rejected, then append the vector to re-run it from the beginning
			targets_position_tmp.push_back(targets_position[marked - (marked > 0)]);
		    }
		...
		std::cout<<"# WARNING : If 'use_previous_catalogues' is non-zero, must be equal to 1 or 2"<<std::endl;
		std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
		std::terminate();
	    }
	}
...
#else
    std::cout<<"Do no forget to disable the -DVELOCITYFIELD option in the Makefile if you want to compute relativistic catalogues"<<std::endl;
    std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
    std::terminate();
#endif
