...
{   
    ...
    public:
        ...
        template <unsigned int Selection = 0, class Octree, unsigned int Dimension = Octree::dimension(), class Element = decltype(Octree::element()), class Index = typename std::tuple_element<0, Element>::type, class Data = typename std::tuple_element<1, Element>::type, class Type = decltype(Data::template type<Selection>()), class = typename std::enable_if<(Dimension == 3)>::type> static inline Data meanAll(const Octree& octree, const Element& element, int level = -1);
        ...
        template <class Parameter, class Data, typename Type, class = typename std::enable_if<Data::types() != 0>::type> static inline Data sistemize(const Parameter& parameters, const Data& data, const Type a, const Type h, const Type omegam, const Type lboxmpch);
        template <class Parameter, class Octree, typename Type, class Element = decltype(Octree::element()), class Data = typename std::tuple_element<1, Element>::type, class = typename std::enable_if<(!std::is_void<Data>::value) && (Octree::dimension() != 0)>::type> static inline unsigned int sistemize(const Parameter& parameters, Octree& octree, const Type h, const Type omegam, const Type lboxmpch);
    ...
    //@{
    public:
        template <typename Integral = unsigned int, typename Real = float, class Parameter, class Octree, class Function, class Element = decltype(Octree::element()), class Index = typename std::tuple_element<0, Element>::type, class Data = typename std::tuple_element<1, 
Element>::type, unsigned int Dimension = Octree::dimension(), class = typename std::enable_if<std::is_convertible<typename std::result_of<Function(Element)>::type, bool>::value>::type> static bool import(const Parameter& parameters, Octree& octree, const std::string& filename, Function&& filter);

       template <typename Type, class Parameter, class Cosmology = std::array<std::vector<Type>, 4>, class Element = typename std::remove_cv<typename std::remove_reference<decltype(std::declval<Cosmology>()[0])>::type>::type, class = typename std::enable_if<std::is_convertible<Element, typename std::remove_cv<typename std::remove_reference<decltype(std::declval<Cosmology>()[0])>::type>::type>::value>::type> static Cosmology acquire(const Parameter& parameters, Type& h, Type& omegam, Type& lboxmpch, const std::string& outfile = std::string());
...
inline bool Input::collide(const Octree& octree, const Index& index, const Sphere& sphere, const Conic& conic)
{
    ...
    return ((Utility::collide(spherified, sphere)) || (Utility::collide(spherified, conic, sphere.extent())));
}
...
template <unsigned int Selection, class Octree, unsigned int Dimension, class Element, class Index, class Data, class Type, class> 
inline Data Input::meanAll(const Octree& octree, const Element& element, int level)
{
    static const int zero = 0;
    static const int one = 1;
    static const int two = 2;
...
inline Data Input::sistemize(const Parameter& parameters, const Data& data, const Type a, const Type h, const Type omegam, const Type lboxmpch)
{
    const Type t = Type(a*a*parameters.mpc)/Type(h*std::hecto::num*std::kilo::num);
...
inline unsigned int Input::sistemize(const Parameter& parameters, Octree& octree, const Type h, const Type omegam, const Type lboxmpch)
{
    Utility::parallelize(octree.begin(), octree.end(), [=, &h, &omegam, &lboxmpch, &parameters](Element& element){std::get<1>(element) = sistemize(parameters, std::get<1>(element), Type(std::get<1>(element).a()), h, omegam, lboxmpch);});
    return octree.size();
}
...
bool Input::import(const Parameter& parameters, Octree& octree, const std::string& filename, Function&& filter)
{
    // Initialization
    ...
    if (stream) {
        ...
        // Loop over levels
	const unsigned int levelmax = parameters.coarseonly ? parameters.ncoarse + 1 : parameters.ncoarse+count.size();
        for (unsigned int ilevel = parameters.ncoarse; ilevel < levelmax; ++ilevel) {
...
bool Input::importhdf5(const Parameter& parameters, unsigned int& rank, const std::array< std::array< double, 3 >, 3 >& rotm1, const double& thetay, const double& thetaz, const Conic& conic, Octree& octree, const std::string& filename, Function&& filter)
{
	...
	std::cout<<"# Coarse level problem between data and 'ncoarse' parameter"<<std::endl;
...
#ifdef VERBOSE
    std::cout<<"# Rank : "<<rank<<" Total number of cells : "<<std::accumulate(count.begin(), count.end(), 0)<<" Filename "<<filename<<std::endl;
    for(unsigned int i = 0; i < count.size(); i++){
	std::cout<<"# Rank : "<<rank<<" Total number of cells at level coarse + "<<i<<" is "<<count[i]<<std::endl;
    }
#endif
    // Loop over levels
    for (unsigned int ilevel = parameters.ncoarse; ilevel < parameters.ncoarse+count.size(); ++ilevel) { 
	...
	cubeNb = cubeNumber[ilevel-parameters.ncoarse];
        // Thread management
        thread = std::thread([=, &a, &size, &selection, &index](){a.resize(size), selection.resize(size); std::iota(selection.begin(), selection.end(), Integral()); index.resize(size); std::iota(index.begin(), index.end(), Integral());});
	thread.join();
	...
	// The shell might be slightly (one coarse step) ahead of the lightcone, in this case the scale factor might be above 1 
	} else{
	...
	if(phi.size() != size || center.size() != Dimension*size || force.size() != Dimension*size){
	    std::cout<<"# Problem size data "<<phi.size()<<" "<<size<<" "<<center.size()<<" "<<Dimension*size<<" "<<force.size()<<" "<<Dimension*size<<std::endl;
	    std::cout<<"# Error at file "<<__FILE__<<" line : "<<__LINE__<<std::endl;
            std::terminate();
	}
	...
	if(parameters.isfullsky == 0 ){
	    ...
	    Utility::parallelize(size, [=, &center, &force, &centertmp, &forcetmp](Integral i){
		center[3*i] = centertmp[3*i]*rotm1[0][0] + centertmp[3*i+1]*rotm1[0][1] + centertmp[3*i+2]*rotm1[0][2];
		center[3*i+1] = centertmp[3*i]*rotm1[1][0] + centertmp[3*i+1]*rotm1[1][1] + centertmp[3*i+2]*rotm1[1][2];
		center[3*i+2] = centertmp[3*i]*rotm1[2][0] + centertmp[3*i+1]*rotm1[2][1] + centertmp[3*i+2]*rotm1[2][2];
		force[3*i] = forcetmp[3*i]*rotm1[0][0] + forcetmp[3*i+1]*rotm1[0][1] + forcetmp[3*i+2]*rotm1[0][2];
		force[3*i+1] = forcetmp[3*i]*rotm1[1][0] + forcetmp[3*i+1]*rotm1[1][1] + forcetmp[3*i+2]*rotm1[1][2];
		force[3*i+2] = forcetmp[3*i]*rotm1[2][0] + forcetmp[3*i+1]*rotm1[2][1] + forcetmp[3*i+2]*rotm1[2][2];
	    });
	}
	    
        // Selection
        Utility::parallelize(index.begin(), index.end(), [=, &filter, &center, &force, &a, &phi, &rho, &selection](Integral& i){selection[i] = filter(Element(Index::template compute<Real, Position, Extent >(octreelevel, center[Dimension*i], center[Dimension*i+1], center[Dimension*i+2]), Data(rho[i], phi[i], std::array<Real, 3>({{force[Dimension*i], force[Dimension*i+1], force[Dimension*i+2]}}), a[i])));}); 
        ...
        std::for_each(selection.begin(), selection.end(), [=, &n](Integral& i){i = i ? ++n : i;});
        thread.join();
        Utility::parallelize(index.begin(), index.end(), [=, &octree, &center, &force, &a, &phi, &rho, &selection](Integral& i){if (selection[i]) octree[selection[i]-1] = Element(Index::template compute<Real, Position, Extent >(octreelevel, center[Dimension*i], center[Dimension*i+1], center[Dimension*i+2]), Data(rho[i], phi[i], std::array<Real, 3>({{force[Dimension*i], force[Dimension*i+1], force[Dimension*i+2]}}), a[i]));}); 
...
bool Input::importfullhdf5(const Parameter& parameters, Octree& octree, const std::string& filename, Function&& filter)
{
	...
	// Read position, potential, force and density from data 
	TReadHDF5::fillVectors_grav(filename, ilevel, ilevel, "gravitational_field_cell", force, "potential_cell", phi, "density_cell", rho, "position_cell", center);
...
/// \return         True on success, false on error.
template <class Position, class Extent, typename Integral, typename Real, class Parameter, class Octree, class Function, class Element, class Index, class Data, unsigned int Dimension, class>
bool Input::importascii(const Parameter& parameters, const std::array< std::array< double, 3 >, 3 >& rotm1, Octree& octree, const std::string& filename, Function&& filter)
{
    ...
    thread = std::thread([=, &size, &level, &center, &force, &a, &phi, &rho, &son](){level.resize(size); center.resize(Dimension*size); force.resize(Dimension*size); a.resize(size); phi.resize(size); rho.resize(size); son.resize(size);});
    thread.join();

    // Read all cells in files
    for(unsigned int i = 0 ; i < size ; ++i){ 
        stream >> level[i] >> center[Dimension*i] >> center[Dimension*i+1] >> center[Dimension*i+2] >> rho[i] >> phi[i] >> force[Dimension*i] >> force[Dimension*i+1] >> force[Dimension*i+2] >> a[i];
    }     
    // For narrow cones, need to rotate position and force
    if(parameters.isfullsky == 0 ){
	std::vector<Real> centertmp = center;
    ...
    std::transform(level.begin(), level.end(), level.begin(), bind2nd(std::plus<Integral>(), levelShift)); 

    // Selection
    thread = std::thread([=, &size, &selection, &index](){selection.resize(size); std::iota(selection.begin(), selection.end(), Integral()); index.resize(size); std::iota(index.begin(), index.end(), Integral());});
    thread.join();
    Utility::parallelize(index.begin(), index.end(), [=, &filter, &level, &center, &force, &a, &phi, &rho, &son, &selection](Integral& i){selection[i] = filter(Element(Index::template compute<Real, Position, Extent >(level[i], center[Dimension*i], center[Dimension*i+1], center[Dimension*i+2]), Data(rho[i], phi[i], std::array<Real, 3>({{force[Dimension*i], force[Dimension*i+1], force[Dimension*i+2]}}), a[i])));}); 
    // Destination
    n = octree.size();
    size = std::accumulate(selection.begin(), selection.end(), n);
    thread = std::thread([=, &octree, &size](){octree.resize(size);});
    std::for_each(selection.begin(), selection.end(), [=, &n](Integral& i){i = i ? ++n : i;});
    thread.join();
    Utility::parallelize(index.begin(), index.end(), [=, &octree, &level, &center, &force, &a, &phi, &rho, &son, &selection](Integral& i){if (selection[i]) octree[selection[i]-1] = Element(Index::template compute<Real, Position, Extent >(level[i], center[Dimension*i], center[Dimension*i+1], center[Dimension*i+2]), Data(rho[i], phi[i], std::array<Real, 3>({{force[Dimension*i], force[Dimension*i+1], force[Dimension*i+2]}}), a[i]));}); 

    // If we only want coarse level, remove refined cells
    if(parameters.coarseonly){
	const unsigned int coarseShift = parameters.ncoarse + levelShift;
	octree.resize(std::distance(std::begin(octree), std::remove_if(std::begin(octree), std::end(octree), [=, &coarseShift](const Element& elem){return std::get<0>(elem).level() != coarseShift;})));
    }
...
/// \return         Container of cosmology evolution and derivatives. Contains t, a, H, int Hdot, Hdot
template <typename Type, class Parameter, class Cosmology, class Element, class> 
Cosmology Input::acquire(const Parameter& parameters, Type& h, Type& omegam, Type& lboxmpch, const std::string& outfile)
{
    // Initialization
    ...
    std::thread thread([=, &line](){std::ifstream input(parameters.evolfile); if (input) {do {line.emplace_back();} while (std::getline(input, line.back())); input.close();}});
    std::ifstream stream;
    std::ofstream outstream;
    std::string string(std::begin(parameters.celldir)+std::min(parameters.celldir.rfind(boxlen), parameters.celldir.size()), std::end(parameters.celldir));
	...
	// Reverse order of data from file (increasing from observer toward the past)
    ...
    if (!((h < zero) || (h > zero)) || !((omegam < zero) || (omegam > zero)) || !((lboxmpch < zero) || (lboxmpch > zero)) || (result.empty())) {
        ...
        if (!magrathea::FileSystem::exist(outfile)) {
            ...
            if (outstream) {
                ...
                outstream<<comment<<space<<h<<space<<omegam<<space<<lboxmpch<<space<<(lboxmpch/h)<<space<<parameters.mpc<<space<<hubble<<space<<(one/hubble)<<space<<(one/(hubble*std::giga::num*year))<<std::right<<std::endl;
                for (unsigned int i = 0; i < size; ++i) {
...
bool Input::load(Octree& octree, const std::string& filename)
{
    ...
    if (ok) {
	// Need to know the size of octree beforehand if reading octree with different structure
        octree.reserve(original+(size/SIZEOFOCTREE)+((size/SIZEOFOCTREE)/factor));
...
Octree& Input::correct(const Parameter& parameters, Octree& octree, Kind&& amin)
{
    // Initialization
    ...
    if (Check >= zero) {
	    ...
	    // Check if we find a cell at coarse level for which the density is not normal
	    ...
	    // For non-normal coarse densities, average over normal neighbours
            Utility::parallelize(counter, [=, &ncoarse, &index, &octree](const unsigned int i){std::get<1>(octree[index[i]]).template data<Selection>() = mean(octree, octree[index[i]], ncoarse);});
    ...
    if (Check >= zero) {
	    ...
	    // Check if the value in .dphidt() is nonnormal(zero, NaN or infinity)
            for (unsigned int n = ncoarse + 1; n <= nmax; ++n) {Utility::parallelize(size, [=, &count, &octree](const unsigned int i){count[i] = ((std::get<0>(octree[i]).level() == n) && (!std::isnormal(std::get<1>(octree[i]).dphidt())));}); 
		// If dphidt() is nonnormal, give the .dphidt() from the parent
                Utility::parallelize(size, [=, &count, &octree](const unsigned int i){if (count[i] > zero) {std::get<1>(octree[i]).dphidt() = std::get<1>(*octree.find(std::get<0>(octree[i]).parent())).dphidt();}}); 
            }
	    // Check if the value in .rho() is nonnormal(zero, NaN or infinity)
            for (unsigned int n = ncoarse + 1; n <= nmax; ++n) {Utility::parallelize(size, [=, &count, &octree](const unsigned int i){count[i] = ((std::get<0>(octree[i]).level() == n) && (!std::isnormal(std::get<1>(octree[i]).rho())));}); 
		// If rho() is nonnormal, give the .rho() from the parent
                Utility::parallelize(size, [=, &count, &octree](const unsigned int i){if (count[i] > zero) {std::get<1>(octree[i]).rho() = std::get<1>(*octree.find(std::get<0>(octree[i]).parent())).rho();}}); 
            }
        } else {
	...
	// Loop over ceclls in octree
		...
		// Loop over sons
		    ...
		    if(ORDER == 0)
		        datinterp = octree.ngp(idxchild.template center<Type,Position,Extent>(0), idxchild.template center<Type,Position,Extent>(1), idxchild.template center<Type,Position,Extent>(2));
		    else if (ORDER == 1)
		        datinterp = octree.cic(idxchild.template center<Type,Position,Extent>(0), idxchild.template center<Type,Position,Extent>(1), idxchild.template center<Type,Position,Extent>(2));
		    else if (ORDER == 2)
		        datinterp = octree.tsc(idxchild.template center<Type,Position,Extent>(0), idxchild.template center<Type,Position,Extent>(1), idxchild.template center<Type,Position,Extent>(2));
		    else{
			std::cout<<"# WARNING : in Makefile, ORDER must be 0, 1 or 2"<<std::endl;
			std::cout<<"# Error at file "<<__FILE__<<", line : "<<__LINE__<<std::endl;
			std::terminate();
		    }
        	    octree.append((datinterp != Data())? Element(idxchild, datinterp) : Element(idxchild, dat));
    		}
            }
        }
    // If we do not want to correct uncomplete refinement levels, then erase them
    } else {
        Utility::parallelize(size, [=, &count, &octree](const unsigned int i){if (!octree.leaf(octree.begin()+i)) {for (unsigned int j = 0; j < Index::sites(); ++j) {count[i] += (octree.find(std::get<0>(octree[i]).child(j)) == octree.end());}}});
        for (unsigned int i = 0; i < size; ++i) {
            if (count[i] > zero) {
    ...
    octree.update();
#ifdef VERBOSE
    std::cout<<"# After correction, Octree size : "<<octree.size()<<" Octree capacity "<<octree.capacity()<<std::endl;
#endif
    return octree;
}
...
/*int Input::example()
{
    ...
    std::cout<<std::setw(width*3)<<"input.import(octree, \"/tmp/file_00001\", 13, 0, [](const decltype(octree.element())&){return true;}) : "                      <<input.import(octree, "/tmp/file_00001", 13, 0, [](const decltype(octree.element())&){return true;})<<std::endl;
