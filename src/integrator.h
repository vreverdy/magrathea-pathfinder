    ...
    do {
        ...
        if (cone.inside(position)) {
            ...
            for (unsigned int idim = 0; idim < Dimension; ++idim) {
                length += (cone.base(idim) - cone.vertex(idim)) *
            ...
            for (unsigned int icone = 0; icone < size; ++icone) {
                ...
                if (&cones[icone] != &cone) {
                    for (unsigned int idim = 0; idim < Dimension; ++idim) {
                        length += (cones[icone].base(idim) - cone.vertex(idim)) *
                                  (position[idim] - cones[icone].vertex(idim));
                    }
                    if (!(length < 0)) {
...
Photon<Type, Dimension> Integrator::launch(const Type xbegin, const Type ybegin,
    ...
    static const Type zero = Type();
    static const Type one = Type(1);
    const Type norm = std::sqrt((xend - xbegin) * (xend - xbegin) +
                                (yend - ybegin) * (yend - ybegin) +
                                (zend - zbegin) * (zend - zbegin));
    Photon<Type, Dimension> result;
...
/// \brief          Launch a photon going from a position to a specified
/// direction. \details        Launches a photon starting from a point and going
/// to specified
///                 direction.
/// \tparam         Type Photon type.
/// \tparam         Dimension Number of space dimension.
...
Container Integrator::launch(const Photon<Type, Dimension> &photon,
    ...
    const Type r = std::sqrt((photon.dxdl() * photon.dxdl()) +
...
Array &Integrator::dphotondl(Array &output, const Array &input,
    ...
    output[dydl] = -(two * dadt / input[a] * input[dtdl] * input[dydl]) +
                   (two / c2 * dphidl * input[dydl]) -
                   (two * data.dphidy() * input[dtdl] * input[dtdl]);
    output[dzdl] = -(two * dadt / input[a] * input[dtdl] * input[dzdl]) +
                   (two / c2 * dphidl * input[dzdl]) -
                   (two * data.dphidz() * input[dtdl] * input[dtdl]);
...
Integrator::integrate(Trajectory &trajectory, const Cosmology &cosmology,
    ...
    if (!trajectory.empty()) {
        ...
        if (!trajectory.empty()) {
            if (std::signbit(trajectory.back().redshift()) ||
                (std::signbit(trajectory.back().a()))) {
                trajectory.pop();
            }
        }
    }
...
// Geodesics integration
/// \brief          Geodesics integration.
/// \details        Integrates the geodesics equation of a photon until some
/// condition. \tparam         Order Octree interpolation order :  0 for NGP, 1
/// for CIC, 2 for TSC
///                 or -1 for an homogeneous universe.
/// \tparam         RK4 Runge-kutta of fourth order or euler.
/// \tparam         Verbose Verbose mode for debug purposes.
...
/// \tparam         Point Point type.
/// \param[in,out]  trajectory Trajectory.
/// \param[in]      interpolation Which type of stop criterion for the
/// integration \param[in]      interpRef Where to stop the integration
/// \param[in]      cosmology Cosmology evolution.
/// \param[in]      octree Octree.
/// \param[in]      vobs Observer peculiar velocity, in SI
/// \param[in]      length Spatial length in SI units.
...
Integrator::integrate(Trajectory &trajectory, const std::string interpolation,
    ...
    if (!trajectory.empty()) {
        ...
        while (data != empty) {
            ...
            // Stop the integration if the photon reaches some value for a given
            // parameter
            if ((interpolation == "r") || (interpolation == "radius")) {
                ...
                if ((photon.x() * kiTarget[0] + photon.y() * kiTarget[1] +
                     photon.z() * kiTarget[2]) > interpRef) {
                    break;
                }
...
magrathea::Evolution<Photon<Type, Dimension>> Integrator::propagate(
    ...
    for (unsigned int itrajectory = 0; itrajectory < ntrajectories;
         ++itrajectory) {
        // Launch photon
        ...
        if (!size) {
            ...
            coord[x] = trajectories[itrajectory].back().x() -
                       trajectories[itrajectory].front().x();
            coord[y] = trajectories[itrajectory].back().y() -
                       trajectories[itrajectory].front().y();
            coord[z] = trajectories[itrajectory].back().z() -
                       trajectories[itrajectory].front().z();
            last[itrajectory] = std::sqrt(coord[x] * coord[x] + coord[y] * coord[y] +
                                          coord[z] * coord[z]);
    ...
    if (ntrajectories > 1) {
        if (interpolation == "redshift") {
            ...
            for (unsigned int itrajectory = 0; itrajectory < ntrajectories;
                ...
                ref[itrajectory].resize(size);
                for (unsigned int istep = 0; istep < size; ++istep) {
                    coord[x] = trajectories[itrajectory][istep].x() -
                               trajectories[itrajectory][first].x();
                    coord[y] = trajectories[itrajectory][istep].y() -
                               trajectories[itrajectory][first].y();
    ...
    if (ntrajectories > 1) {
        ...
        for (unsigned int istep = 0; istep < size; ++istep) {
            for (unsigned int itrajectory = 1; itrajectory < ntrajectories;
                 ++itrajectory) {
    ...
    // Finalization
    return ntrajectories > 0 ? trajectories[0]
...
std::array<std::vector<double>, 4>(); magrathea::SimpleHyperOctree<double,
magrathea::SimpleHyperOctreeIndex<unsigned long long int, 3>, Gravity<float, 3>
> octree(0, 2); magrathea::HyperSphere<3> sphere =
magrathea::HyperSphere<3>::unit(); magrathea::Evolution<Photon<double, 3> >
trajectory; Photon<double, 3> photon; Cone<> cone(beg, end, 0.42);
    ...
    std::cout<<std::setw(width)<<"Integrator() : "                            ;
Integrator(); std::cout<<std::endl; std::cout<<std::setw(width)<<"integrator =
...
std::cout<<std::endl;
    ...
    std::cout<<std::setw(width*2)<<"integrator.launch(sphere, cones[0], engine,
distribution) : "                       <<integrator.launch(sphere, cones[0],
engine, distribution)<<std::endl;
    std::cout<<std::setw(width*2)<<"integrator.launch(sphere, cones[0], cones,
engine, distribution) : "                <<integrator.launch(sphere, cones[0],
cones, engine, distribution)<<std::endl;
    std::cout<<std::setw(width*2)<<"integrator.launch(beg[0], beg[1], beg[2],
end[0], end[1], end[2]) : "               <<integrator.launch(beg[0], beg[1],
beg[2], end[0], end[1], end[2])<<std::endl;
    std::cout<<std::setw(width*2)<<"integrator.launch(photon, 3, 0.42,
0.1).size() : "                                  <<integrator.launch(photon, 3,
0.42, 0.1).size()<<std::endl;

    // Computation
    std::cout<<std::endl;
    std::cout<<std::setw(width*2)<<"Computation : " <<std::endl;
    std::cout<<std::setw(width*2)<<"integrator.dphotondl(array, array,
cosmology, octree, one, one, one)[0] : "         <<integrator.dphotondl(array,
array, cosmology, octree, one, one, one)[0]<<std::endl;

    // Evolution
    std::cout<<std::endl;
    std::cout<<std::setw(width*3)<<"Evolution : " <<std::endl;
    std::cout<<std::setw(width*3)<<"integrator.integrate(trajectory, cosmology,
octree, one, one).size() : " <<integrator.integrate(trajectory, cosmology,
octree, one, one).size()<<std::endl;
    std::cout<<std::setw(width*3)<<"integrator.propagate(photon, 3, 0.42, 0.1,
\"a\", cosmology, octree, one, one).size()" <<integrator.propagate(photon, 3,
0.42, 0.1, "a", cosmology, octree, one, one).size()<<std::endl;
