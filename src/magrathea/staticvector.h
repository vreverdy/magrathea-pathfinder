...
namespace magrathea {
    ...
    class StaticVector final
      : public StaticVectorizer<unsigned int, Size, StaticVector, Type, Size> {
        // Setup
    public:
        using StaticVectorizer<unsigned int, Size, magrathea::StaticVector, Type, Size>::operator=;
    ...
    public:
        ...
        inline StaticVector(const StaticVector<FundamentalType, Size>& source);
        template<typename OtherType = Type, class... Misc, class = typename std::enable_if<std::is_convertible<OtherType, Type>::value>::type>
        inline StaticVector(const std::initializer_list<OtherType>& source, const Misc&... misc);
        template<class... Misc, class = typename std::enable_if<sizeof...(Misc) != 0>::type>
        explicit inline StaticVector(const Misc&... misc);
        //@}
    ...
    int StaticVector<Type, Size>::example() {
        ...
        std::cout << std::setw(width) << "operator>>(iss, inumbers1) : ";
        iss >> inumbers1;
        std::cout << std::endl;
        std::cout << std::setw(width) << "operator<<(std::cout, inumbers1) : " << inumbers1 << std::endl;
