...
namespace magrathea {
    ...
    class NArray final
      : public AbstractNArray<unsigned int, Size, NArray, Type, Size> {
        // Setup
    public:
        using AbstractNArray<unsigned int, Size, NArray, Type, Size>::operator=;
    ...
    int NArray<Type, Size>::example() {
        ...
        std::cout << std::setw(width * 2) << "std::for_each(big.begin(), big.end(), [](double& x){x *= 2;}) : ";
        std::for_each(big.begin(), big.end(), [](double& x) { x *= 2; });
        std::cout << std::endl;
        std::cout << std::setw(width * 2) << "std::for_each(big.begin(), big.end(), [](double x){std::cout<<x<<' ';}) : ";
        std::for_each(big.begin(), big.end(), [](double x) { std::cout << x << ' '; });
        std::cout << std::endl;
        std::cout << std::setw(width * 2) << "std::for_each(big.cbegin(), big.cend(), [](double x){std::cout<<x<<' ';}) : ";
        std::for_each(big.cbegin(), big.cend(), [](double x) { std::cout << x << ' '; });
        std::cout << std::endl;
        std::cout << std::setw(width * 2) << "std::for_each(big.rbegin(), big.rend(), [](double& x){x /= 2;}) : ";
        std::for_each(big.rbegin(), big.rend(), [](double& x) { x /= 2; });
        std::cout << std::endl;
        std::cout << std::setw(width * 2) << "std::for_each(big.rbegin(), big.rend(), [](double x){std::cout<<x<<' ';}) : ";
        std::for_each(big.rbegin(), big.rend(), [](double x) { std::cout << x << ' '; });
        std::cout << std::endl;
        std::cout << std::setw(width * 2) << "std::for_each(big.crbegin(), big.crend(), [](double x){std::cout<<x<<' ';}) : ";
        std::for_each(big.crbegin(), big.crend(), [](double x) { std::cout << x << ' '; });
        std::cout << std::endl;
