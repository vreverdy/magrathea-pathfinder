...
namespace magrathea {
    ...
    public:
        ...
        template<unsigned int Subdimension = Dimension, typename Iterator, class = typename std::enable_if<(Subdimension + 1 == Dimension) && (Subdimension >= 1) && (Subdimension <= 2) && (std::is_convertible<typename std::remove_cv<typename std::remove_reference<decltype(*std::declval<Iterator>())>::type>::type, Vector>::value)>::type>
        std::pair<Scalar, Scalar> uniform(const Iterator& first, const Iterator& last) const;
        //@}
    ...
    template<class Crtp, unsigned int Dimension, class Vector, typename Scalar>
    inline AbstractHyperSphere<Crtp, Dimension, Vector, Scalar>::~AbstractHyperSphere() = default;
    // -------------------------------------------------------------------------- //
