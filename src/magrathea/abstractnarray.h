...
namespace magrathea {
    ...
    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::operator=;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::min;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::max;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::null;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::eq;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::ne;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::modify;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::apply;

    public:
        using StaticVectorizer<Kind, Size, Crtp, Type, Parameters...>::reduce;
    ...
    public:
        ...
        inline AbstractNArray(const AbstractNArray<Kind, Size, Crtp, FundamentalType, Parameters...>& source);
        template<typename OtherType = Type, class... Misc, class = typename std::enable_if<std::is_convertible<OtherType, Type>::value>::type>
        inline AbstractNArray(const std::initializer_list<OtherType>& source, const Misc&... misc);
        template<class... Misc, class = typename std::enable_if<sizeof...(Misc) != 0>::type>
        explicit inline AbstractNArray(const Misc&... misc);
        //@}
    ...
    public:
        ...
        inline unsigned int index(const IteratorType& it, typename std::iterator_traits<IteratorType>::iterator_category* = nullptr) const;
        //@}
    ...
    public:
        ...
        inline bool eq(const GenericType& rhs, const Type& tolerance) const;
        template<class GenericType>
        inline bool ne(const GenericType& rhs, const Type& tolerance) const;
        //@}
    ...
    public:
        ...
        inline const Type& amin(const Mask& bitmask = Mask()) const;
        template<class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        inline const Type& amax(const Mask& bitmask = Mask()) const;
        ...
        inline Return mean(const Coefficient& coefficient = Coefficient(1), const Mask& bitmask = Mask()) const;
        ...
        inline Return sigma(const Correction& correction = Correction(0), const Coefficient& coefficient = Coefficient(1), const Mask& bitmask = Mask()) const;
        //@}
        ...
        //@{
    public:
        template<typename Return = Type, typename First = Type, class... Types, class... Args, class = typename std::enable_if<(std::is_convertible<typename std::decay<First>::type, typename std::decay<Type>::type>::value) && (sizeof...(Types) == sizeof...(Args))>::type>
        inline Crtp<Type, Parameters...>& modify(Return (*f)(First, Types...), Args&&... args);
        template<typename Return = Type, typename First = Type, class... Types, class... Args, class Mask, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) && (std::is_convertible<typename std::decay<First>::type, typename std::decay<Type>::type>::value) && (sizeof...(Types) == sizeof...(Args))>::type>
        inline Crtp<Type, Parameters...>& modify(const Mask& bitmask, Return (*f)(First, Types...), Args&&... args);
        template<typename Return = Type, typename First = Type, class... Types, class... Args, class = typename std::enable_if<(std::is_convertible<typename std::decay<First>::type, typename std::decay<Type>::type>::value) && (sizeof...(Types) == sizeof...(Args))>::type>
        inline Crtp<Return, Parameters...> apply(Return (*f)(First, Types...), Args&&... args) const;
        template<typename Return = Type, typename First = Type, class... Types, class... Args, class Mask, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) && (std::is_convertible<typename std::decay<First>::type, typename std::decay<Type>::type>::value) && (sizeof...(Types) == sizeof...(Args))>::type>
        inline Crtp<Return, Parameters...> apply(const Mask& bitmask, Return (*f)(First, Types...), Args&&... args) const;
        //@}
    ...
    public:
        ...
        inline bool unicity(Function&& f = Function(), const Mask& bitmask = Mask()) const;
        template<class Function = std::equal_to<Type>, class Mask = std::true_type, class = typename std::enable_if<(!std::is_base_of<Vectorizer, typename std::decay<Function>::type>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline unsigned int distinct(Function&& f = Function(), const Mask& bitmask = Mask()) const;
        //@}
    ...
    public:
        ...
        inline Crtp<Type, Parameters...>& arrange(Function&& f = Function(), Indexes&& indexes = Indexes(), const Mask& bitmask = Mask());
        template<class Function = std::less<Type>, class Indexes = Type, class Mask = std::true_type, class Pair = std::pair<Type, typename std::remove_const<typename std::remove_reference<decltype(Vectorizer::get(std::declval<Indexes>(), 0))>::type>::type>, class = typename std::enable_if<(!std::is_base_of<Vectorizer, typename std::decay<Function>::type>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline Crtp<Type, Parameters...> sort(Function&& f = Function(), Indexes&& indexes = Indexes(), const Mask& bitmask = Mask()) const;
        template<class Function = std::less<Type>, class Indexes = Type, class Mask = std::true_type, class Pair = std::pair<Type, typename std::remove_const<typename std::remove_reference<decltype(Vectorizer::get(std::declval<Indexes>(), 0))>::type>::type>, class = typename std::enable_if<(!std::is_base_of<Vectorizer, typename std::decay<Function>::type>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline Crtp<Type, Parameters...>& aarrange(Function&& f = Function(), Indexes&& indexes = Indexes(), const Mask& bitmask = Mask());
        template<class Function = std::less<Type>, class Indexes = Type, class Mask = std::true_type, class Pair = std::pair<Type, typename std::remove_const<typename std::remove_reference<decltype(Vectorizer::get(std::declval<Indexes>(), 0))>::type>::type>, class = typename std::enable_if<(!std::is_base_of<Vectorizer, typename std::decay<Function>::type>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline Crtp<Type, Parameters...> asort(Function&& f = Function(), Indexes&& indexes = Indexes(), const Mask& bitmask = Mask()) const;
        ...
        inline Crtp<Type, Parameters...>& rearrange(const Indexes& indexes = Indexes());
        template<class Indexes = Type, class Pair = std::pair<typename std::remove_const<typename std::remove_reference<decltype(Vectorizer::get(std::declval<Indexes>(), 0))>::type>::type, unsigned int>>
        inline Crtp<Type, Parameters...> resort(const Indexes& indexes = Indexes()) const;
        //@}
    ...
    public:
        template<class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        inline Crtp<Type, Parameters...> abs(const Mask& bitmask = Mask()) const;
        template<int Exponent = 1>
        inline Crtp<Type, Parameters...> pow() const;
        ...
        inline Crtp<Type, Parameters...> pow(const GenericType& rhs, const Mask& bitmask = Mask()) const;
        template<int Degree = 1>
        inline Crtp<Type, Parameters...> rt() const;
        template<class GenericType, class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        inline Crtp<Type, Parameters...> rt(const GenericType& rhs, const Mask& bitmask = Mask()) const;
        template<unsigned int Base = 0>
        inline Crtp<Type, Parameters...> log() const;
        template<class GenericType, class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        inline Crtp<Type, Parameters...> log(const GenericType& rhs, const Mask& bitmask = Mask()) const;
        //@}
    ...
    public:
        ...
        inline NormType norm(const Mask& bitmask = Mask()) const;
        template<unsigned int Degree = 2, typename NormType = typename std::conditional<std::is_floating_point<Type>::value, Type, double>::type, class Mask = std::true_type, class = typename std::enable_if<(std::is_arithmetic<NormType>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline Crtp<Type, Parameters...>& renormalize(const Mask& bitmask = Mask());
        template<unsigned int Degree = 2, typename NormType = typename std::conditional<std::is_floating_point<Type>::value, Type, double>::type, class Mask = std::true_type, class = typename std::enable_if<(std::is_arithmetic<NormType>::value) && ((std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value))>::type>
        inline Crtp<Type, Parameters...> normalize(const Mask& bitmask = Mask()) const;
        //@}
    ...
    public:
        ...
        template<class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        static inline Crtp<Type, Parameters...> value(const Type& source = Type(), const Mask& bitmask = Mask());
        template<class Mask = std::true_type, class = typename std::enable_if<(std::is_base_of<Vectorizer, Mask>::value) || (std::is_same<std::true_type, Mask>::value)>::type>
        static inline Crtp<Type, Parameters...> random(const Type& lowest = 0, const Type& highest = 1, const Mask& bitmask = Mask());
        ...
        static inline Crtp<Type, Parameters...> random(Engine&& engine, Distribution&& distribution, const Mask& bitmask = Mask());
        ...
        static inline Crtp<Type, Parameters...> indexed(Function&& f = std::forward<unsigned int>, const Mask& bitmask = Mask());
        ...
        static inline Crtp<Type, Parameters...> progressive(const Type& first = Type(0), const Type& step = Type(1), Function&& f = Function(), const Mask& bitmask = Mask());
        //@}
    ...
    template<typename Kind, unsigned int Size, template<typename, Kind...> class Crtp, typename Type, Kind... Parameters>
    inline AbstractNArray<Kind, Size, Crtp, Type, Parameters...>::~AbstractNArray() = default;
    // -------------------------------------------------------------------------- //
